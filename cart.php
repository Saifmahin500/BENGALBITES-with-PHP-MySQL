...
$st = $DB_con->prepare("SELECT id FROM carts WHERE user_id = ? AND
status = 'open' LIMIT 1");
$st->execute([$userId]);
...
if (file_exists(__DIR__ . '/partials/header.php')) include __DIR__ .
	'/partials/header.php';
if (file_exists(__DIR__ . '/partials/navbar.php')) include __DIR__ .
	'/partials/navbar.php';
?>
<!doctype html>
<html lang="en">
...
<head>
	<?php if (!defined('BOOTSTRAP_LOADED')): // fallback if header didn't
		// load CSS 
	?>
		<meta charset="utf-8">
						...
						<div class="p-4">
							<div class="card empty-card text-center p-5">
								...
								<a class="btn btn-primary" href="<?=
																	htmlspecialchars($BASE) ?>/index.php">Browse Products</a>
							</div>
						</div>
					<?php else: ?>
								...
								<tbody id="cartBody">
									<?php foreach ($items as $it):
										$img = (!empty($it['product_image']) &&
											file_exists(__DIR__ . '/admin/uploads/' . $it['product_image']))
											? 'admin/uploads/' . $it['product_image']
											: 'assets/images/placeholder.png';
										$line = (float)$it['unit_price'] * (int)$it['qty'];
									?>
										<tr data-item-id="<?= (int)$it['item_id'] ?>">
											<td><img src="<?= htmlspecialchars($img) ?>"
													class="cart-img" alt=""></td>
											<td>
												<div class="font-weight-500"><?=
																				htmlspecialchars($it['product_name']) ?></div>
											</td>
											<td class="text-right">$ <span class="unit"><?=
																						number_format($it['unit_price'], 2) ?></span></td>
											<td>
												<input type="number" class="form-control
form-control-sm qty" min="1"
													value="<?= (int)$it['qty'] ?>">
											</td>
											<td class="text-right">$ <span
													class="line-total"><?= number_format($line, 2) ?></span></td>
											<td class="text-center">
												<button class="btn btn-sm btn-outline-danger
remove" title="Remove">&times;</button>
											</td>
										</tr>
									<?php endforeach; ?>
								</tbody>
								<tfoot>
									<tr>
										...
										<th class="text-right">$ <span id="grandTotal"><?=
																						number_format($grand, 2) ?></span></th>
										<th></th>
									</tr>
								</tfoot>
							</table>
				...
				<div class="card summary-card">
					...
					<div class="card-body">
						<div class="d-flex justify-content-between mb-2">
							<span>Subtotal</span>
							<strong>$ <span id="sumSubtotal"><?=
																number_format($grand, 2) ?></span></strong>
						</div>
						...
						<div class="coupon-wrap mb-2">
							<div class="custom-control custom-checkbox mb-2">
								<input type="checkbox" class="custom-control-input"
									id="chkCoupon">
								<label class="custom-control-label" for="chkCoupon">Have
									Coupon?</label>
							</div>
							...
							<div id="couponArea" class="d-none">
								<div class="form-inline">
									<input type="text" name="couponCode" id="couponCode"
										class="form-control form-control-sm mr2 coupon-input"
										placeholder="Enter Coupon Code">
									<button type="button" id="btnApplyCoupon" class="btn
btn-sm btn-outline-primary">Apply</button>
								</div>
						...
						<div class="d-flex justify-content-between h5">
							<span>Total</span>
							<strong>$ <span id="sumGrand"><?= number_format($grand, 2)
															?></span></strong>
						</div>
					</div>
...
mb-2" placeholder="Your Name" value="<?= $_SESSION['user_name'] ?>"
									required>
								<input type="text" name="phone" class="form-control
mb-2" placeholder="Your Phnone Number" value="<?=
												$_SESSION['user_phone'] ?>">
								<input type="text" name="area" class="form-control mb-2"
									placeholder="Input Your Area" required>
	...
	if (file_exists(__DIR__ . '/partials/footer.php')) include __DIR__ .
		'/partials/footer.php';
	?>

	<?php if (!defined('BOOTSTRAP_JS_LOADED')): // fallback if footer
		// didn't load JS 
	?>
	...
	<script>
		...
		function recalcTotals() {
			...
			document.querySelectorAll('#cartBody tr').forEach(function(tr) {
				const unit = parseFloat((tr.querySelector('.unit')?.textContent ||
					'0').replace(/,/g, ''));
		...
		document.addEventListener('input', function(e) {
			...
			fetch('ajax/cart_update_qty.php', {
					...
					body: 'item_id=' + encodeURIComponent(itemId) + '&qty=' +
				...
				.then(d => {
					...
					if (__coupon.code) applyCoupon(__coupon.code);
					else recalcTotals();
		...
		function applyCoupon(code) {
			...
			fetch('ajax/apply_coupon.php', {
				...
				if (!d || !d.ok) {
					...
					$('#couponRow').addClass('d-none');
					$('#couponMsg').removeClass('text-success').addClass('text-danger').text(d &&
						d.msg ? d.msg : 'Invalid Coupon.');
					recalcTotals();
					return;
				}
				...
				__coupon = {
					...
					amount: parseFloat(d.discount || 0)
				};

				$('#couponPercent').text(__coupon.percent.toFixed(2));
				$('#couponAmount').text(__coupon.amount.toFixed(2));
				$('#couponRow').removeClass('d-none');
				$('#couponMsg').removeClass('text-danger').addClass('text-success').text('Couponapplied  successfully');
				recalcTotals();
			}).catch(() => {
				$('#couponMsg').removeClass('text-success').addClass('text-danger').text('ErrorApplying Coupon');
			});
